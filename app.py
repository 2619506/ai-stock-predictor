...
# Helper: Pure Pandas RSI Calculation (Prevents needing extra TA libraries)
...
# --- SIDEBAR CONFIGURATION & NEW AI TOOLS ---
st.sidebar.header("🛸 Control Matrix")
ticker = st.sidebar.text_input("Stock Ticker", value="AAPL").upper()
days_back = st.sidebar.slider("Historical Window (Days)", min_value=60, max_value=365, value=180)
...
with st.spinner(f"Extracting technical matrices for {ticker}..."):
...
if df is None or df.empty:
    ...
    st.subheader(f"Technical Candlestick Array: {ticker}")
    fig = go.Figure()
    ...
    # Toggle interface for Top/Worst
    show_worst = st.checkbox("🔄 Toggle View: Show Market Laggards (Worst Performing)", value=False)
    
    # Pre-calculated performance data proxy for major standard tokens
    market_universe = {
    ...
    if st.button("Execute Neural Simulation"):
        with st.spinner("Processing prompt structures..."):
            try:
                ...
                # Dynamic prompt generation based on sidebar tools selection
                if ai_tool == "📊 Quant Signal Analyzer":
                    prompt_focus = f"Analyze these indicators: RSI is currently at {latest_rsi:.1f}, price is relative to 20 SMA at {latest_sma20:.2f}. Deliver an academic evaluation supporting a '{tech_signal}' action plan detailing specific entry or exit boundary mechanics."
                elif ai_tool == "🧠 Sentiment Catalyst Simulator":
                    prompt_focus = "Evaluate potential underlying psychological factors influencing trading volume shifts and retail momentum spikes. Map this against behavioral finance theory."
                else:
                    prompt_focus = f"Simulate a macro-economic scenario where interest rates change by +/- 50bps. How would this macro shift interact with this stock's calculated Beta coefficient of {beta:.2f}?"
                ...
                Conclude with a explicit statement that the analysis is systemic AI generation and does not represent legally binding financial execution advice.
                """
